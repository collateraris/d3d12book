...
PanoToCubemapPSO::PanoToCubemapPSO()
{
    ...
    m_DefaultUAV = GetApp().AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 5);
    UINT descriptorHandleIncrementSize = GetApp().GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
const RootSignature& PanoToCubemapPSO::GetRootSignature() const
{
    return m_RootSignature;
}

const Microsoft::WRL::ComPtr<ID3D12PipelineState>& PanoToCubemapPSO::GetPipelineState() const
{
