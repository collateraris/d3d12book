...
std::shared_ptr<Window> Application::GetWindowByName(const std::wstring& windowName)
{
    ...
    WindowNameMap::iterator iter = m_WindowByName.find(windowName);
    if (iter != m_WindowByName.end())
    {
        window = iter->second;
    }
...
{
    WindowMap::iterator windowIter = m_Windows.find(hWnd);
    if (windowIter != m_Windows.end())
    {
        WindowPtr pWindow = windowIter->second;
        m_WindowByName.erase(pWindow->GetWindowName());
        m_Windows.erase(windowIter);
    }
}

void Application::GetWindowByhWnd(HWND hWnd, WindowPtr& window)
{
    WindowMap::iterator iter = m_Windows.find(hWnd);
...
bool Application::IsWindowsEmpty() const
{
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (pWindow)
    {
        ...
        case WM_DESTROY:
        {
            ...
            // window maps.
            gs_pSingelton->RemoveWindow(hwnd);

            if (gs_pSingelton->IsWindowsEmpty())
            {
